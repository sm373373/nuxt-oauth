...
const SECRET_KEY = global.env.SECRET_KEY || crypto.randomBytes(32).toString('hex')

// Helper function to create an HMAC signature
function signState(data) {
  return crypto.createHmac('sha256', SECRET_KEY).update(data).digest('hex')
}

// Helper function to verify the signature
function verifyState(data, signature) {
  return signState(data) === signature
}
...
Handler.prototype.authenticateCallbackToken = async function authenticateCallbackToken() {
  ...
  try {
    ...
    const stateData = decodedState.data
    const signature = decodedState.sig

    // Verify the integrity of the state parameter
    if (!verifyState(stateData, signature)) {
      throw new Error("Invalid state signature detected")
    }

    redirectUrl = JSON.parse(stateData).redirectUrl
